...
The check command runs the portable Node test suite and bundles the ES module source into one userscript. It writes generated output to both:
    ...
    dist/insta-media-capture.user.js
    release/insta-media-capture.user.js
...
Watch mode updates only `dist/insta-media-capture.user.js`; run `npm run build` before committing a release artifact.

The legacy userscript `@namespace` is intentionally retained to avoid changing
the script's historical identity. The v1.2 and v1.2.1 repository and artefact
renames require a one-time reinstall because GitHub's former raw-content URL
does not reliably follow a renamed repository or file. Future releases use the
...
After `npm run check`, install `release/insta-media-capture.user.js` in Tampermonkey and validate one example of each available Instagram content shape:
